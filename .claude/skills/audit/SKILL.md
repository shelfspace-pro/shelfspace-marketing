...
- **Voice violations**: forbidden tokens — `automate` / `automated` / `automatically`; `scan-based trading` / `SBT`; `Diem`; `pilot` / `trial` / `60-day`; `<br>` inside headings (h1/h2/h3); CTAs not pointing to `/contact`; "the platform does X" instead of "we do X"; passive constructions where direct fits
...
- **Schema validity**: JSON-LD blocks present and well-formed; FAQPage answers as complete standalone statements (so AI search models can pull them verbatim); BreadcrumbList for blog/docs pages; Article schema on blog posts
...
- **Convention drift**: design tokens used (`--green-deep`, `--slate-600`, etc.) not hardcoded hex; existing primitives reused (`page-hero`, `article-glance`, `compare-table`) not reinvented
