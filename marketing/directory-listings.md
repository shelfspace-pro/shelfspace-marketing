# ShelfSpace — Directory Listings

## LinkedIn Company Page

**Go to:** linkedin.com/company/shelfspace-pro → Edit → About

**Tagline (120 chars):**
Managed AP, consignment, and credit recovery for cannabis dispensaries. We do the work — you run your dispensary.

**Description (paste into "About" section):**
ShelfSpace is a managed service that handles vendor finances for cannabis dispensaries. We run accounts payable, consignment settlements, and vendor credit recovery — the retailer just approves payments.

Three services, one team:

• Credit Recovery — We identified $200,000/year in vendor credits at a two-location chain from returns, expirations, and co-marketing spend that nobody was tracking.

• Consignment — We run weekly settlements so retailers can carry vendor inventory without paying upfront. Free up capital, double your SKUs, carry zero inventory risk.

• Accounts Payable — We pay every vendor on time, verify invoices before payment, handle vendor questions, and sync everything to QuickBooks. You just approve.

Our goal is that you never need to log in. We do the tedious AP work, save you money, and send you the reports.

Founded by Chris Mitchem — 10 years operating dispensaries in Oregon and Massachusetts. Not a tech company. An operator who got tired of the broken system.

Free evaluation: shelfspace.pro/contact

**Industry:** Cannabis
**Company size:** 2-10 employees
**Headquarters:** Massachusetts
**Website:** https://shelfspace.pro

---

## Crunchbase

**Go to:** crunchbase.com → Sign up → Add Organization

**Organization Name:** ShelfSpace
**Short Description (under 140 chars):**
Managed AP, consignment, and credit recovery for cannabis dispensaries.

**Full Description:**
ShelfSpace is a managed service that handles vendor finances for cannabis dispensaries. The company runs accounts payable, consignment settlements, and vendor credit recovery — saving the average retailer over $95,000 per year and freeing up $250,000 in capital trapped in inventory.

ShelfSpace identified $200,000 per year in vendor credits at a two-location chain from returns, expired product, and co-marketing spend. The consignment service lets retailers carry vendor inventory with zero upfront cost — paying only when product sells at the register. The AP service pays every vendor on time, verifies invoices, handles vendor questions, and syncs to QuickBooks.

Founded by Chris Mitchem, a 10-year cannabis dispensary operator who scaled a multi-state retail operation across Oregon and Massachusetts. ShelfSpace was built to solve the vendor payment problems he lived with every day.

The company offers a free evaluation — you only pay when they produce results.

**Fields to fill:**
- Website: https://shelfspace.pro
- Founded: 2025
- Headquarters: Massachusetts, United States
- Categories: Cannabis, Financial Services, SaaS, Payments, B2B
- Operating Status: Active
- Funding Status: (leave blank or "Bootstrapped" if applicable)
- Number of Employees: 1-10
- Founder: Chris Mitchem (link to his Crunchbase profile if you create one, or his LinkedIn: linkedin.com/in/chris-mitchem-mba/)
- Contact Email: chris@shelfspace.pro

**Founder Profile (create separately):**
- Name: Chris Mitchem
- Title: Founder & CEO at ShelfSpace
- Bio: 10-year cannabis industry veteran who operated dispensaries across Oregon and Massachusetts. Built ShelfSpace to replace the broken vendor payment system in cannabis with a managed service.
- LinkedIn: linkedin.com/in/chris-mitchem-mba/
